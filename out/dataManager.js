...
class DataManager {
    ...
    async getAppData() {
        ...
        try {
            // 检查是否启用工作区模式
            let savedData;
            const globalData = this.context.globalState.get(DataManager.STORAGE_KEYS.APP_DATA);
            const workspaceData = this.context.workspaceState.get(DataManager.STORAGE_KEYS.WORKSPACE_DATA);
     ...
     * 切换工作区模式
     */
    async toggleWorkspaceMode(enable) {
    ...
    async getSystemStatus() {
        ...
        const syncConfig = this.context.globalState.get(DataManager.STORAGE_KEYS.SYNC_CONFIG);
    ...
    async restoreFromLatestBackup() {
        try {
            const backupHistory = this.context.globalState.get(DataManager.STORAGE_KEYS.BACKUP_HISTORY, []);
    ...
    async importData() {
        try {
            ...
            // 询问是否覆盖现有数据
            const choice = await vscode.window.showWarningMessage('导入数据将覆盖当前所有数据，是否继续？', { modal: true }, '继续导入', '取消');
            if (choice === '继续导入') {
                // 先创建当前数据的备份
    ...
    async syncFromCloud() {
        try {
            ...
            if (cloudData && cloudData.data) {
                // 询问是否覆盖本地数据
                const choice = await vscode.window.showWarningMessage('从云端同步将覆盖本地数据，是否继续？', { modal: true }, '继续同步', '取消');
                if (choice === '继续同步') {
    ...
    async updateBackupHistory(backupPath, timestamp) {
        const history = this.context.globalState.get(DataManager.STORAGE_KEYS.BACKUP_HISTORY, []);
    ...
    async syncToGitHub(data, config) {
        // GitHub Gist API 实现
        const gistData = {
    ...
    async syncToGitee(data, config) {
        console.log('同步到Gitee:', data);
    }
    async syncFromGitee(config) {
        console.log('从Gitee同步');
        return null;
    }
    async syncToCustomAPI(data, config) {
        console.log('同步到自定义API:', config.apiUrl);
    }
    async syncFromCustomAPI(config) {
