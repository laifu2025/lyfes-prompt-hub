...
const promptHubProvider_1 = require("./promptHubProvider");
const dataManager_1 = require("./dataManager");
let dataManager;
function activate(context) {
    console.log('Prompt Hub 扩展已激活');
    // 初始化数据管理器
    dataManager = new dataManager_1.DataManager(context);
    // 创建 Prompt Hub Provider
    const promptHubProvider = new promptHubProvider_1.PromptHubProvider(context.extensionUri, dataManager);
    // 注册 webview provider
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('promptHubView', promptHubProvider, {
        webviewOptions: {
            retainContextWhenHidden: true
        }
    }));
    // 注册命令
    context.subscriptions.push(vscode.commands.registerCommand('promptHub.openPanel', () => {
        vscode.commands.executeCommand('workbench.view.extension.promptHub');
    }));
    context.subscriptions.push(vscode.commands.registerCommand('promptHub.refresh', () => {
        promptHubProvider.refresh();
    }));
    // 数据管理相关命令
    context.subscriptions.push(vscode.commands.registerCommand('promptHub.exportData', async () => {
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('promptHub.importData', async () => {
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('promptHub.createBackup', async () => {
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('promptHub.restoreBackup', async () => {
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('promptHub.setupCloudSync', async () => {
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('promptHub.syncToCloud', async () => {
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('promptHub.syncFromCloud', async () => {
        try {
    ...
    // 工作区模式相关命令
    context.subscriptions.push(vscode.commands.registerCommand('promptHub.toggleWorkspaceMode', async () => {
        try {
            ...
            const choice = await vscode.window.showInformationMessage(`当前存储模式：${storageInfo.location}`, `${action}工作区模式`, '取消');
    ...
    context.subscriptions.push(vscode.commands.registerCommand('promptHub.showStorageInfo', async () => {
        try {
            const storageInfo = await dataManager.getStorageInfo();
